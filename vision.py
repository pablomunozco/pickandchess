...
def object_detect():
    ...
    for i in range(8):
        for j in range(8):
            ...
            cv2.imwrite('casilla' + str(i) + str(j) + '.jpg', tablero[anc * i:anc * i + anc - 1, alt * j:alt * j + alt - 1])
            imageCasilla = cv2.imread('casilla' + str(i) + str(j) + '.jpg')

            gray = cv2.cvtColor(imageCasilla, cv2.COLOR_BGR2GRAY)


            thresh = cv2.threshold(mask[anc * i:anc * i + anc - 1, alt * j:alt * j + alt - 1], 220, 255, cv2.THRESH_BINARY)[1]
